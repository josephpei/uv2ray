...
/**
 * 生成服务器子菜单
 * @param {*Array<Object>} configs v2ray配置集合
 * @param {*Number} selectedIndex 选中的v2ray配置的索引
 */
  ...
  const submenus = Object.keys(groups).map(key => {
    ...
    return {
      ...
      submenu: groupedConfigs.map(config => {
        return {
          id: config.id,
          label: `${config.emoji || ''}${config.ps || config.add} (${config.add}:${config.port})`,
          type: 'checkbox',
...
function generateMenus (appConfig) {
  const base = [
    ...
    { label: 'PAC', submenu: [{ label: '更新PAC', click: handler.updatePac }] },
    {
      label: '服务器',
